...
- 2026-09-07 — Edición de vacantes company — Una vacante propia ofrece “Editar vacante” y reutiliza el formulario de publicación precargado desde `GET /job/{id}`. El guardado usa `PUT /job/{id}` con el contrato completo y conserva `opensAt` cuando ya es una fecha histórica; al confirmar retorna al detalle real — Pendiente validación local y commit.
- 2026-09-07 — Jerarquía y responsive transversal — El candidato autenticado entra a `GET /job` desde Buscar empleos; company conserva Mis vacantes como entrada y Publicar vacante como acción primaria. Se retiró el fondo animado global, se unificaron inputs, botones, cards, estados vacíos y foco visible con los tokens Llanqui. La navegación móvil mantiene drawer táctil y no crea datos ni rutas nuevas — `npm run type-check` correcto — Pendiente validación visual local y commit.
## Recomendaciones personalizadas en búsqueda de empleo

- La vista de candidato `/job-search` consulta `GET /api/v1/profile/me` y usa únicamente las habilidades, puestos de experiencia y formación reales retornados por el perfil.
- Con esas señales llama `POST /api/v1/recommendations/specific` y cruza `source_url` con las vacantes reales de `GET /api/v1/job`; no se crean tarjetas ni porcentajes ficticios.
- El candidato puede ajustar puesto/habilidades, modalidad y salario mínimo. Estas respuestas se envían al recomendador para recalcular la búsqueda actual.
- Pendiente de backend/recommendation: no existe un endpoint autenticado para guardar feedback persistente (por ejemplo, "no me interesa") ni para usarlo en el reentrenamiento del modelo. La interfaz no afirma que las preferencias se aprendan de forma permanente.
